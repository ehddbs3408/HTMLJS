...
App.get("/record",async(req:Request,res:Response)=>{
    //서버 랭킹에서 가장높은 레벨을 달성한 유저들 중 3명을 가져오는데
    //만약 동일 레벨이라면 달성 날까지
    //SELECT * FROM score ORDER BY level DESC,time LIMIT 0,5
    let sql:string = `SELECT * FROM score ORDER BY level DESC,time LIMIT 0,3`;

    let [rows,fieldInfo] : [Score[],FieldPacket[]] = await ConPool.query(sql);
    
    res.json({rows,msg:"로딩됨"});
})
...
App.post("/record", async (req:Request, res:Response) =>{
    ...
    let {username, level} = req.body; //post로 전송된 데이터는 body로
